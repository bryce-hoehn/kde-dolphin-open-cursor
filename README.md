# Dolphin: Open in Cursor

A context menu entry for Dolphin to open Cursor at the current location.
...
Move the file `openCursor.desktop` into the [Service Menus folder](https://develop.kde.org/docs/apps/dolphin/service-menus/):
- KDE Plasma 5: `~/.local/share/kservices5/ServiceMenus`
- KDE Plasma 6: `~/.local/share/kio/servicemenus`

Set as executable:
```bash
chmod +x ./openCursor.desktop
```

## Customization

By default, the extension works on directories (i.e. `inode/directory`). If you prefer the extension to work on all file types, edit the file `openCursor.desktop` by replacing the line `MimeType=inode/directory` with `MimeType=all/all`.
